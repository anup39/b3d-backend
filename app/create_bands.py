...
GDAL_CONFIG = {'GDAL_TIFF_INTERNAL_MASK': True, 'GDAL_TIFF_OVR_BLOCKSIZE': 256,
               'GDAL_CACHEMAX': 536870912, 'GDAL_SWATH_SIZE': 1073741824}
COG_PROFILE = {
    "count": 1,
    "driver": "GTiff",
    "interleave": "pixel",
...
def process_band(src, output_folder, raster_id, raster_data_instance, color, band_number, progress, multiband=False):
    ...
    output_file = f"{output_folder}/{raster_id}_{color}.tif"
    ...
    if in_memory:
        ...
        fileobj = tempfile.NamedTemporaryFile(
            dir=str(output_folder), suffix=".tif")
        fileobj.close()
    ...
    copy(dst, str(output_file), copy_src_overviews=True,
        ...
        with rasterio.Env(**GDAL_CONFIG):
            with rasterio.open(file_path) as src:
                ...
                for band_number, color in [(1, "red"), (2, "green"), (3, "blue")]:
                    progress = process_band(
                        src, output_folder, raster_id, raster_data_instance, color, band_number, progress)
